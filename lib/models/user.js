...
class User {
  static async findByEmail(email) {
    // Also select the role and profilePictureUrl
  ...
  // Method for NextAuth to find or create a user based on OAuth profile
  static async findOrCreateForOAuth({ email, name, image }) {
    // `image` here is the profile picture URL from OAuth provider
    return new Promise((resolve, reject) => {
      // Try to find the user by email
      const findQuery = 'SELECT id, name, email, role, profilePictureUrl FROM users WHERE email = ?';
      pool.query(findQuery, [email], async (error, results) => {
        if (error) {
          return reject(error);
        }
        ...
        if (results && results.length > 0) {
          ...
          // Optionally, update name or image if they have changed in OAuth provider
          // For now, just return the user.
          // We might want to ensure profilePictureUrl is updated if image is new and different.
          if (image && existingUser.profilePictureUrl !== image) {
            await User.updateProfilePictureUrl(existingUser.id, image);
            existingUser.profilePictureUrl = image; // Reflect update in returned object
          }
          return resolve(existingUser);
        } else {
          // User not found, create a new one
          // For OAuth users, password is not set directly.
          // The 'password' column in your DB must allow NULL or have a default
          // if you don't set a placeholder. For this example, assuming it allows NULL
          // or createUser handles it. Our current createUser requires a password.
          // So, we need a way to create a user without a password or with a placeholder.

          // Let's create a placeholder password for now, or ensure DB schema allows NULL for password.
          // This is a simplification. Ideally, distinguish OAuth users.
          const placeholderPassword = `oauth_user_${Date.now()}`; // Not meant for login
          const hashedPassword = await hash(placeholderPassword, 10); // Still hash it
          const defaultRole = 'user';

          const insertQuery = 'INSERT INTO users (name, email, password, role, profilePictureUrl) VALUES (?, ?, ?, ?, ?)';
          const values = [name, email, hashedPassword, defaultRole, image];
          ...
          pool.query(insertQuery, values, (insertError, insertResults) => {
            ...
            // Fetch the newly created user to return it (including default role, etc.)
            const fetchNewUserQuery = 'SELECT id, name, email, role, profilePictureUrl FROM users WHERE id = ?';
            pool.query(fetchNewUserQuery, [newUserId], (fetchError, fetchResults) => {
              if (fetchError) {
